                        ...
                        <%@ page import="dao.PaymentDAO" %>
                            <%@ page import="model.BookingModel" %>
                                <%@ page import="model.PaymentModel" %>
                                    ...
                                    <% // Check if user is logged in (all photographers can access) if
                                        (session.getAttribute("photographerId")==null) {
                                        response.sendRedirect(request.getContextPath() + "/jsp/photographer/login.jsp"
                                        ); return; } // Set current page for sidebar
                                        request.setAttribute("currentPage", "reports" ); // Get date range parameters
                                        String startDateStr=request.getParameter("startDate"); String
                                        endDateStr=request.getParameter("endDate"); String
                                        reportType=request.getParameter("reportType"); // Default to last 30 days if not
                                        specified SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd"); Calendar
                                        cal=Calendar.getInstance(); Date endDate=cal.getTime();
                                        cal.add(Calendar.DAY_OF_MONTH, -30); Date startDate=cal.getTime(); if
                                        (startDateStr !=null && !startDateStr.isEmpty()) { try {
                                        startDate=sdf.parse(startDateStr); } catch (Exception e) { // Use default } } if
                                        (endDateStr !=null && !endDateStr.isEmpty()) { try {
                                        endDate=sdf.parse(endDateStr); } catch (Exception e) { // Use default } } if
                                        (reportType==null || reportType.isEmpty()) { reportType="all" ; } // Get data
                                        from DAOs BookingDAO bookingDAO=new BookingDAO(); PaymentDAO paymentDAO=new
                                        PaymentDAO(); List<BookingModel> allBookings = bookingDAO.getAllBookings();
                                        List<PaymentModel> allPayments = paymentDAO.getAll();
                                            ...
                                            List<BookingModel> filteredBookings = new ArrayList<BookingModel>();
                                                    ...
                                                    String dateOnly = bookDateStr.length() >= 10 ?
                                                    bookDateStr.substring(0, 10) : bookDateStr;
                                                    ...
                                                    Map<String, Double> revenueByDay = new TreeMap<String, Double>();
                                                            Map<String, Integer> bookingsByDay = new TreeMap<String,
                                                                    Integer>();
                                                                    ...
                                                                    String bookDateStr = b.getBookDate();
                                                                    if (bookDateStr != null && bookDateStr.length() >=
                                                                    10) {
                                                                    String dateKey = bookDateStr.substring(0, 10);
                                                                    Double currentRevenue = revenueByDay.get(dateKey);
                                                                    revenueByDay.put(dateKey, (currentRevenue != null ?
                                                                    currentRevenue : 0.0) + b.getTotalPrice());

                                                                    Integer currentCount = bookingsByDay.get(dateKey);
                                                                    bookingsByDay.put(dateKey, (currentCount != null ?
                                                                    currentCount : 0) + 1);
                                                                    }

                                                                    if ("Confirmed".equals(b.getBookStatus()))
                                                                    confirmedBookings++;
                                                                    else if ("Pending".equals(b.getBookStatus()))
                                                                    pendingBookings++;
                                                                    else if ("Completed".equals(b.getBookStatus()))
                                                                    completedBookings++;

                                                                    if ("Indoor".equalsIgnoreCase(b.getPackageCateg()))
                                                                    indoorBookings++;
                                                                    else if
                                                                    ("Outdoor".equalsIgnoreCase(b.getPackageCateg()))
                                                                    outdoorBookings++;
                                                                    }
                                                                    ...
                                                                    bookDateStr.substring(0, 10) : bookDateStr;
                                                                    Date bookDate = sdf.parse(dateOnly);
                                                                    if (!bookDate.before(prevStartDate) &&
                                                                    bookDate.before(startDate)) {
                                                                    prevBookings++;
                                                                    ...
                                                                    request.setAttribute("verifiedPayments",
                                                                    verifiedPayments);
                                                                    request.setAttribute("pendingPayments",
                                                                    pendingPayments);
                                                                    request.setAttribute("bookingChange",
                                                                    bookingChange);
                                                                    request.setAttribute("revenueChange",
                                                                    revenueChange);
                                                                    request.setAttribute("confirmedBookings",
                                                                    confirmedBookings);
                                                                    request.setAttribute("pendingBookings",
                                                                    pendingBookings);
                                                                    request.setAttribute("completedBookings",
                                                                    completedBookings);
                                                                    request.setAttribute("indoorBookings",
                                                                    indoorBookings);
                                                                    request.setAttribute("outdoorBookings",
                                                                    outdoorBookings);
                                                                    request.setAttribute("filteredBookings",
                                                                    filteredBookings);
                                                                    request.setAttribute("revenueByDay", revenueByDay);
                                                                    request.setAttribute("bookingsByDay",
                                                                    bookingsByDay);
                                                                    %>
                                                                    ...
                                                                    <head>
                                                                        ...
                                                                        <link
                                                                            href="https://fonts.googleapis.com/css2?family=Roboto:wght@300;400;500;700&family=Poppins:wght@300;400;500;600;700&family=Josefin+Sans:wght@300;400;500;600;700&display=swap"
                                                                            rel="stylesheet">
                                                                        <link
                                                                        ...
                                                                        <link
                                                                            ...
                                                                            rel="stylesheet">
                                                                        <link
                                                                            href="${pageContext.request.contextPath}/assets/css/photographer-style.css?v=2"
                                                                            rel="stylesheet">
                                                                        <script
                                                                                ...
                                                                                <div class="stats-grid">
                                                                                    <div class="stat-box bookings">
                                                                                        ...
                                                                                        <div class="stat-value">
                                                                                            ${totalBookings}</div>
                                                                                        <div
                                                                                            class="stat-change ${bookingChange >= 0 ? 'positive' : 'negative'}">
                                                                                            <i
                                                                                                class="bi ${bookingChange >= 0 ? 'bi-arrow-up' : 'bi-arrow-down'}"></i>
                                                                                            ${bookingChange >= 0 ? '+' :
                                                                                            ''}${bookingChange} from
                                                                                            last period
                                                                                        </div>
                                                                                    ...
                                                                                    <div class="stat-box revenue">
                                                                                        ...
                                                                                        <div class="stat-label">Total
                                                                                            Revenue</div>
                                                                                        <div class="stat-value">RM
                                                                                            <fmt:formatNumber
                                                                                                value="${totalRevenue}"
                                                                                                pattern="#,##0.00" />
                                                                                        </div>
                                                                                        <div
                                                                                            class="stat-change ${revenueChange >= 0 ? 'positive' : 'negative'}">
                                                                                            <i
                                                                                                class="bi ${revenueChange >= 0 ? 'bi-arrow-up' : 'bi-arrow-down'}"></i>
                                                                                            ${revenueChange >= 0 ? '+' :
                                                                                            ''}RM
                                                                                            <fmt:formatNumber
                                                                                                value="${revenueChange}"
                                                                                                ...
                                                                                                <c:forEach var="booking"
                                                                                                    ...
                                                                                                    <c:if
                                                                                                        ...
                                                                                                        <tr>
                                                                                                            ...
                                                                                                            <td>${booking.clientName}
                                                                                                            </td>
                                                                                                            <td>
                                                                                                                ${booking.packageName}<br>
                                                                                                                <span
                                                                                                                    class="${booking.packageCateg == 'Indoor' ? 'badge-indoor' : 'badge-outdoor'}">${booking.packageCateg}</span>
                                                                                                            </td>
                                                                                                            <td>${booking.bookDate}
                                                                                                            </td>
                                                                                                            <td>RM
                                                                                                            ...
                                                                                                            <td>
                                                                                                                <span
                                                                                                                    class="badge ${booking.bookStatus == 'Confirmed' ? 'bg-success' : (booking.bookStatus == 'Pending' ? 'bg-warning text-dark' : 'bg-secondary')}">${booking.bookStatus}</span>
                                                                                                            </td>
                                                                                                        </tr>
                                                                                                    </c:if>
                                                                                                </c:forEach>
                                                                                                <c:if
                                                                                                    test="${empty filteredBookings}">
                                                                                                    <tr>
                                                                                                        <td colspan="6"
                                                                                                            class="text-center py-4 text-muted">
                                                                                                            <i
                                                                                                                class="bi bi-inbox display-4"></i>
                                                                                                            <p
                                                                                                                class="mt-2">
                                                                                                                No
                                                                                                                bookings
