                                            ...
                                            <c:forEach var="item" items="${pendingPayments}">
                                                <tr>
                                                    <td>
                                                        ...
                                                        <small>
                                                            <fmt:formatDate value="${item.bookDate}"
                                                                pattern="dd MMM yyyy" />
                                                        </small>
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        ${item.packageName}<br>
                                                        <span
                                                            class="badge ${item.packageCateg == 'Indoor' ? 'bg-info' : 'bg-success'}">${item.packageCateg}</span>
                                                    </td>
                                                    <td>RM
                                                    ...
                                                    <td>
                                                        <c:if
                                                            test="${not empty item.depoRef && (item.payStatus == 'submitted' || item.payStatus == 'pending')}">
                                                            <span class="badge bg-warning text-dark">Deposit</span>
                                                        </c:if>
                                                        <c:if
                                                            test="${not empty item.fullRef && item.payStatus == 'submitted'}">
                                                    ...
                                                    <td>
                                                        <c:if test="${not empty item.depoReceipt}">
                                                            <button class="btn btn-sm btn-outline-info mb-1"
                                                                onclick="viewReceipt('${item.depoReceipt}', 'Deposit Receipt')">
                                                                <i class="bi bi-receipt"></i> Deposit
                                                            </button>
                                                        </c:if>
                                                        <c:if test="${not empty item.fullReceipt}">
                                                            <button class="btn btn-sm btn-outline-info"
                                                                onclick="viewReceipt('${item.fullReceipt}', 'Full Payment Receipt')">
                                                                <i class="bi bi-receipt"></i> Full
                                                            </button>
                                                        </c:if>
                                                        <c:if
                                                            test="${empty item.depoReceipt && empty item.fullReceipt}">
                                                    ...
                                                    <td>
                                                        ...
                                                        <button class="btn btn-sm btn-outline-danger"
                                                            onclick="showRejectModal(${item.payId})">
                                        ...
                                        <tbody>
                                            <c:forEach var="item" items="${completedPayments}">
                                                <tr>
                                                    ...
                                                    <td class="text-success fw-bold">RM
                                                        <fmt:formatNumber value="${item.paidAmount}"
                                                            pattern="#,##0.00" />
                                                    </td>
                                                    <td>
                                                        <c:if test="${not empty item.depoRef}">
                                                    ...
                                                    <td>
                                                        <c:if test="${item.verifiedDate != null}">
                                                            <fmt:formatDate value="${item.verifiedDate}"
                                                                pattern="dd/MM/yyyy" />
                                                        </c:if>
                                                    </td>
                                                </tr>
                                        ...
                                        <tbody>
                                            <c:forEach var="item" items="${confirmHistory}">
                                                <tr>
                                                    ...
                                                    <td class="text-success">RM
                                                        <fmt:formatNumber value="${item.paidAmount}"
                                                            pattern="#,##0.00" />
                                                    </td>
                                                    <td class="${item.remAmount > 0 ? 'text-danger' : 'text-success'}">
                                                        RM
                                                        <fmt:formatNumber value="${item.remAmount}"
                                                            pattern="#,##0.00" />
                                                    </td>
                                                    <td>
                                                        <c:choose>
                                                            ...
                                                            <c:otherwise>
                                                                <span
                                                                    class="badge bg-secondary">${item.payStatus}</span>
                                                            </c:otherwise>
                                                        </c:choose>
                                                    </td>
                            ...
                            <form action="${pageContext.request.contextPath}/payment" method="post" id="verifyForm">
                                <div class="modal-body">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="verifyType" id="verifyType" class="form-select" required
                                            onchange="updateVerifyAmount()">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="number" name="verifyAmount" id="verifyAmount" class="form-control"
                                            step="0.01" required>
                    ...
                    <div class="modal-dialog modal-lg modal-dialog-centered">
                        <div class="modal-content">
                            <div class="modal-header bg-info text-white">
                                <h5 class="modal-title" id="receiptModalTitle"><i class="bi bi-receipt"></i> View
                                    Receipt</h5>
                                <button type="button" class="btn-close btn-close-white"
                                    data-bs-dismiss="modal"></button>
                            </div>
                            <div class="modal-body text-center p-4">
                                <img id="receiptImage" src="" class="img-fluid rounded shadow"
                                    style="max-height: 500px;" alt="Receipt">
